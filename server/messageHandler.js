...
async function kuralResim({
  ...
  setTimeout(async () => {
    ...
    // Aynı batch'te fiyatlı resim varsa (albüm senaryosu: race condition önleme) → koru
    if (typeof getK2BatchHasFiyat === 'function' && getK2BatchHasFiyat(delUserId)) return;
...
async function kural3Check({
  ...
  io.emit('log', { type: 'deleted', user: userName || userPhone, group: groupName });
  io.emit('deleted_ads_updated', { total: deletedAdsLog.length });
  // K3 batch tracker'ını 5dk sonra temizle (paidTime süresi kadar, FIVE_MIN zaten tanımlı)
